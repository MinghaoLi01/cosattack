...
class SDEdit():
    def __init__(self, net = None, dm = None):
        if dm != None:
            self.dm = dm
            self.c = ''
        else:
    ...
    def edit_list(self, x, restep=None, t_list=[0.01, 0.05, 0.1, 0.2, 0.3], c=None, noise = None):
        l = [(x.cpu() + 1) /2]
        for t in t_list:
            x_return = self.edit(x, t, restep=restep,c=c,noise=noise)
            l.append((x_return + 1 ) /2 )
        return l

    @torch.no_grad()
    def edit(self, x, guidance, restep=None, from_z=False, c=None,noise = None):
        # create DDIM Sampler
        # guidance from 0-1
...
class Linf_PGD():
    ...
    def pgd_sds_latent(self, z, net, c, label=None, random_start=False, delta_score=False, name='test', diff_pgd=False, using_target=False, target_image=None, target_rate=None):
        ...
        if random_start:
            ...
            z_adv = z.clone().detach() + (torch.rand(*z.shape)*2*self.eps-self.eps).cuda()
        else:

...
def init(epsilon: int = 16, steps: int = 100, alpha: int = 1, 
    ...
    if base is None:
        base = 'configs/stable-diffusion/v1-inference-sd1-4.yaml'
...
def LDMAttack(eps=8,steps=100,input_size=512,mode='sds',img_path=
         'test_images/test',output_path='out/',alpha=1,g_mode='-',diff_pgd=[False, 0.15, 'ddim100'],using_target=False,device=0,rate=5,block_num =1):
    ...
    output_path = output_path #+ f'/{mode_name}_eps{eps}_steps{steps}_gmode{g_mode}'
    if diff_pgd[0]:
    ...
    config = init(epsilon=eps, alpha=alpha, steps=steps, 
    ...
    for image_path in tqdm(img_paths):
        ...
        for i in tqdm(range(block_num)):
            for j in tqdm(range(block_num)):
                ...
                output_image[bls*i: bls*i+bls, bls*j: bls*j + bls]  = infer(img_block, config, tar_block, diff_pgd=diff_pgd, using_target=using_target, device=device)
        
        output = Image.fromarray(output_image.astype(np.uint8))
        
        # time_start_sdedit = time.time()
        # si(edit_one_step, output_path + f'{file_name}_onestep.jpg')
        # si(edit_multi_step, output_path + f'{file_name}_multistep.jpg')
        # print('SDEdit takes: ', time.time() - time_start_sdedit)
        
        
        output_name = output_path + f'/{file_name}.jpg'

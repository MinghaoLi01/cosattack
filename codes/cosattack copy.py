...
def attack(tp=999,steps=1, Estepsize=1/255.0, Esteps=100, Eeps=8/255.0,device=0,
    ...
    for i in tqdm(imgs[:]):
        ...
        for _ in range(Esteps):
            ...
            z_grad = getTargetZ(z,z_raw,dm,t,steps,c,target_z,noise,tlst=tlst_)
...
def main():
    ...
    # SDEexp(img_path = output, output_path = f"exp/sde/{outpath}/"+exp_name,clean_path=clean_pth,xlsxname=f"{scorefile}_{outpath}")
    ...
    # outpath = f"advdm"
    # output = LDMAttack(mode='advdm',img_path=clean_pth,output_path=f"exp/out/{outpath}/"+exp_name,g_mode='+')
